...
export const LanguageProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getLoc = <T extends Record<string, any>>(obj: T | undefined | null, fieldPrefix: string): string => {
    ...
    if (obj[altKey] !== undefined && obj[altKey] !== null && String(obj[altKey]).trim() !== '') {
    ...
    const aliases: Record<string, string[]> = {
      name: ['title', 'name'],
      title: ['title', 'name'],
      shortDescription: ['shortDesc', 'shortDescription', 'desc', 'fullDesc'],
      shortDesc: ['shortDesc', 'shortDescription', 'desc', 'fullDesc'],
      description: ['fullDesc', 'desc', 'shortDesc', 'shortDescription'],
      fullDesc: ['fullDesc', 'desc', 'shortDesc', 'shortDescription'],
      location: ['location', 'address']
    };

    const alternativePrefixes = aliases[fieldPrefix] || [];
    for (const altPrefix of alternativePrefixes) {
      if (altPrefix === fieldPrefix) continue;
      const k1 = `${altPrefix}${suffix}`;
      if (obj[k1] !== undefined && obj[k1] !== null && String(obj[k1]).trim() !== '') {
        return String(obj[k1]);
      }
      const k2 = `${altPrefix}${altSuffix}`;
      if (obj[k2] !== undefined && obj[k2] !== null && String(obj[k2]).trim() !== '') {
        return String(obj[k2]);
      }
    }
