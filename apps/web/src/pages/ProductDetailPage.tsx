        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-14 mb-16">
          ...
          <div className="space-y-4">
            <div className="relative h-[380px] sm:h-[480px] bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden shadow-2xl">
              <img 
                src={product.images?.[selectedImageIndex] || product.images?.[0] || 'https://images.unsplash.com/photo-1508514177221-188b1cf16e9d?auto=format&fit=crop&q=80&w=800'} 
            ...
            <div className="p-5 rounded-2xl bg-zinc-900/80 border border-zinc-800/80 flex items-center justify-between">
              <div>
                ...
                <span className="text-2xl font-black text-emerald-400 mt-1 block">
                  {product.priceUZS ? `${product.priceUZS.toLocaleString()} SO‘M` : (product.priceUSD ? `$${product.priceUSD}` : t('requestQuote'))}
                </span>
              </div>
            ...
            {product.specs && product.specs.length > 0 && (
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
                {product.specs.map((sp) => (
                  <div key={sp.id} className="p-3 bg-zinc-900 border border-zinc-800 rounded-xl">
                    <span className="text-zinc-500 block text-[11px]">{getLoc(sp, 'key')}</span>
                    <span className="font-bold text-white mt-0.5 block">{getLoc(sp, 'value')}</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <div className="lg:col-span-2 space-y-8">
            <div>
              ...
              <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden divide-y divide-zinc-800/80">
                {(product.specs || (product as any).specifications || []).map((spec: any, idx: number) => (
                  <div key={idx} className="p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                    <span className="font-semibold text-zinc-400 w-1/3">
                      {getLoc(spec, 'key') || spec.keyUz || spec.keyRu}
                    </span>
                    <span className="font-bold text-white w-2/3">
                      {getLoc(spec, 'value') || spec.valueUz || spec.valueRu}
                    </span>
                  </div>
                ))}
              </div>
