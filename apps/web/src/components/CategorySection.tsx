          ...
          {activeCategories.map((cat, idx) => {
            ...
            return (
              <button
                ...
                className="group text-left relative min-h-[310px] sm:min-h-[340px] rounded-3xl overflow-hidden bg-white border border-[#DDE7E4] shadow-[0_16px_55px_rgba(14,35,31,.07)] hover:border-[#08B4A5]/45 hover:-translate-y-1 hover:shadow-[0_24px_70px_rgba(8,180,165,.12)] transition-all duration-300"
              >
                <div className="absolute inset-y-0 right-0 w-[54%] bg-[radial-gradient(circle_at_center,rgba(8,180,165,.12),transparent_68%)]" />
                <img
                  src={cat.imageUrl || '/images/products/tanso-bosimsiz-main.png'}
                  alt={getLoc(cat, 'name')}
                  className="absolute right-[-2%] bottom-[-4%] w-[52%] h-[88%] object-contain object-center transition-transform duration-500 group-hover:scale-[1.045]"
                />

                <div className="relative z-10 p-6 sm:p-8 w-[58%] min-h-[310px] sm:min-h-[340px] flex flex-col">
                  <div className="flex items-center gap-3">
                    <span className="grid place-items-center w-10 h-10 rounded-xl bg-[#08B4A5]/10 text-[#08B4A5]"><Icon className="w-4.5 h-4.5" /></span>
                    <span className="text-[10px] font-black uppercase tracking-[.15em] text-[#F58A36]">0{idx + 1} · {count} {language === 'ru' ? 'тов.' : 'mahsulot'}</span>
                  </div>
                  <div className="mt-auto pt-10">
                    <h3 className="text-xl sm:text-2xl font-black tracking-[-0.025em] text-[#0E1715] group-hover:text-[#078F84] transition-colors">{getLoc(cat, 'name')}</h3>
                    <p className="mt-2 text-xs sm:text-sm leading-relaxed text-[#63716E] line-clamp-3">{getLoc(cat, 'description')}</p>
