...
export const OrdersPage: React.FC = () => {
  ...
  if (orders.length === 0) {
    return (
      <div className="px-4 pt-16 pb-24 flex flex-col items-center text-center">
        ...
        <h2 className="text-base font-bold text-[var(--ink)] mb-1">So'rovlar yo'q</h2>
        <p className="text-sm text-[var(--muted)] max-w-xs">Katalogdan mahsulot tanlab buyurtma bersangiz, shu yerda ko'rinadi.</p>
      </div>
    );
  }
  ...
  return (
    <div className="px-4 pt-4 pb-24">
      <h1 className="text-lg font-extrabold text-[var(--ink)] mb-4">Mening so'rovlarim</h1>
        ...
        {orders.map((order) => (
          <div key={order.id} className="card p-4">
            ...
            <p className="text-xs text-[var(--muted)] pt-2 border-t border-[var(--border)]">
              Mas'ul menejer {order.phone} raqamiga qo'ng'iroq qiladi.
            </p>
          </div>
        ))}
      </div>
