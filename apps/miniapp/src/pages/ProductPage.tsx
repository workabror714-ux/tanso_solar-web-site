  ...
  return (
    <div className="pb-28">
      <div className="sticky top-0 z-30 bg-[var(--paper)]/95 backdrop-blur border-b border-[var(--border)] px-4 py-3 flex items-center gap-3">
        ...
        <p className="text-sm font-bold text-[var(--ink)] truncate">{product.titleUz}</p>
      </div>

      <div className="aspect-square bg-[var(--teal-tint)] flex items-center justify-center">
        {product.images?.[activeImg] ? (
          <img src={product.images[activeImg]} alt={product.titleUz} className="w-full h-full object-contain p-6" />
        ) : null}
      </div>
      ...
      <div className="px-4 pt-2">
        {category && <span className="badge badge-teal mb-2">{category.nameUz}</span>}
        <h1 className="text-xl font-extrabold text-[var(--ink)] leading-tight mb-2">{product.titleUz}</h1>

        <div className="flex items-center justify-between mb-4">
          {price ? (
            <span className="font-mono-num font-extrabold text-2xl text-[var(--ink)]">{price}</span>
          ) : (
            <span className="text-sm text-[var(--muted)]">Narx so'rov bo'yicha</span>
          )}
          {product.inStock ? (
            <span className="badge badge-teal">Mavjud</span>
          ) : (
            <span className="badge" style={{ color: 'var(--danger)', borderColor: 'rgba(179,67,47,.3)', background: 'var(--danger-tint)' }}>Tugagan</span>
          )}
        </div>

        {product.shortDescUz && (
          <p className="text-sm text-[var(--muted)] leading-relaxed mb-5">{product.shortDescUz}</p>
        )}

        {product.specs?.length > 0 && (
          <div className="mb-6">
            <p className="text-xs font-bold uppercase tracking-wide text-[var(--muted)] mb-2">Texnik xususiyatlari</p>
            <div className="card divide-y divide-[var(--border)] overflow-hidden">
              {product.specs.map((spec) => (
                <div key={spec.id} className="flex items-center justify-between px-3.5 py-2.5 text-sm">
                  <span className="text-[var(--muted)]">{spec.keyUz}</span>
                  <span className="font-semibold text-[var(--ink)] text-right ml-3">{spec.valueUz}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      {product.inStock && (
        <div
          className="fixed bottom-16 left-0 right-0 z-40 bg-[var(--surface)] border-t border-[var(--border)] px-4 py-3 flex items-center gap-3"
